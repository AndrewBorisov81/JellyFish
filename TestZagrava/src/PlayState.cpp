...
bool PlayState::onEnter()
{
  ...
  int W, H;
  SDL_GetRendererOutputSize(TheGame::Instance()->getRenderer(), &W, &H);
  m_screenSize.w = W;
  m_screenSize.h = H;
  
  //Test jelly fish
  Point posJellyFish(500, 500);
...
bool PlayState::checkMinDist(const Vector2D& beginPoint, const Vector2D& endPoint, const Size& objectSize)
{
  ...
  float dist = Math::distance(beginPoint.getX(), beginPoint.getY(), endPoint.getX(), endPoint.getY());
...
GameObject* PlayState::getHitObject(const Vector2D& pos, const std::vector<GameObject*>& gameObjects)
{
  ...
  for (auto &el : gameObjects)
  {
    if (el->isClickable())
    {
      ...
      isHitObject = Math::isInside(A, B, C, D, M);
    }
