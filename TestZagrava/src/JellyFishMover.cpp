...
void JellyFishMover::move()
{
  ...
  case State::MOVE:
  {
    ...
    m_curDist = Math::distance(m_endPoint.getX(), m_endPoint.getY(), m_currentPos.getX(), m_currentPos.getY());
...
{
  float distToEndPoint = Math::distance(endPoint.getX(), endPoint.getY(), beginPoint.getX(), beginPoint.getY());
...
void JellyFishMover::calculateLengthSegments(std::vector<float>& m_lengthSegments, const std::vector<Vector2D>& toMovePoints, Vector2D beginPoint)
{
  ...
  float distFirstSegment = Math::distance(beginPoint.getX(), beginPoint.getY(), firstToMovePoint.getX(), firstToMovePoint.getY());
  ...
  for (int i = 0; i < toMovePoints.size() - 1; i++)
  {
    ...
    curDist = Math::distance(point1.getX(), point1.getY(), point2.getX(), point2.getY());
    totalDist += curDist;
...
void JellyFishMover::setState(State state)
{
  ...
  case State::STOP:
    ...
    if (m_indexCurrentPoint < m_toMovePoints.size())
    {
      ...
      m_prevDist = Math::distance(m_endPoint.getX(), m_endPoint.getX(), m_currentPos.getX(), m_currentPos.getY());
      m_maxDist = Math::distance(m_endPoint.getX(), m_endPoint.getY(), m_beginPoint.getX(), m_beginPoint.getY());
